<div class="bg-white rounded-lg shadow-md flex flex-col sm:flex-row w-full max-w-[550px] h-auto sm:h-[310px]">
    ...
    <div class="flex justify-center items-center w-full p-4">
        <div class="w-full">
            <div class="bg-[#FFEEEC] text-[#F5A89F] p-2 rounded-full w-max mx-auto sm:mx-0 mb-4 sm:mb-2">
                <span class="font-semibold text-lg">Consultar Actividades</span>
            </div>
            ...
            <div class="mb-4">
                ...
                <label for="ciclo" class="block text-sm font-medium text-gray-700">Ciclo Escolar</label>
                <select id="ciclo" wire:model="ciclo"
                    class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">
                    <option value="">Ciclo Escolar</option>
                    @foreach ($ciclos as $ciclo)
                        <option value="{{ $ciclo->idCicloEscolar }}">{{ $ciclo->ciclo }} - {{ $ciclo->year }}</option>
                    @endforeach
                </select>
            </div>
